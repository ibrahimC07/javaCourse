...
//        İşleme başlarken bir önceki değer olmadığı için bu değer identity parametresinde tanımlanmaktadır.
        ...
        public static void elemanlariCarp(List<Integer> sayilar) {
            ...
            System.out.println("sayilar.stream().reduce(1,(a,b)-> a*b) = " +
                    sayilar.stream()
                            .filter(MyMethods::ciftElemaniBul)//method ref kullanildi kendi class imizda
                            // create ettigimiz seed methodu kulandik
                            .reduce(1, (a, b) -> a * b));
            //method referans ile
            System.out.println(sayilar.stream().filter(MyMethods::ciftElemaniBul)
    ...
    public static void teklerinKareBuyuktenKucuge(List<Integer> sayi) {
        sayi.stream().//liste aaskisa alindi
                ...
                .forEach(MyMethods::yazdir);//seed method kullanilarakl method ref ile yazdirldi
    }
    // Task : List elemanlarini alafabetik buyuk harf ve  tekrarsiz print ediniz.
    public static void alfabetikBuyukHarfTekrarsiz(List<String> yemek) {
            yemek.stream().distinct().map(t->t.toUpperCase()).forEach(MyMethods::yazdir);
    }
    ...
    public static void sonHarfeGoreTersSira(List<String> yemek) {//yukarida tanimladigim listlerden calisiacam
       yemek.stream().sorted(Comparator.comparing(MyMethods::sonKarakteriAl).reversed()).forEach(MyMethods::yazdir);
    }
    ...
    public static void karkterEnFazla(List<String> yemek) {
       ...
       List<String> sonuc = yemek.stream().sorted(Comparator.comparing(String::length).reversed()).limit(1).collect(Collectors.toList());
